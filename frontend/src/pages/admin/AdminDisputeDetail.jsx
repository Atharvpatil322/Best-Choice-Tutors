  ...
  const handleResolve = async () => {
    ...
    if (outcome === 'PARTIAL_REFUND') {
      ...
      if (!Number.isInteger(amount) || amount <= 0) {
        setResolveError('Enter a valid refund amount in paise');
        return;
      }
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 px-4 py-8">
        <div className="container mx-auto max-w-2xl">
          <Card>
            <CardContent className="p-6">
              <p className="text-center text-destructive">{error || 'Dispute not found.'}</p>
              <div className="mt-4 flex justify-center">
              ...
              {outcome === 'PARTIAL_REFUND' && (
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Full amount: {dispute.amountInPaise ?? '—'} paise ({formatAmount(dispute.amountInPaise)})
                  </p>
                </div>
              )}
