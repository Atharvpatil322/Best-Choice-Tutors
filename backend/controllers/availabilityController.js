  ...
  try {
    ...
    if (!tutor) {
      return res.status(403).json({
        message: 'Tutor profile not found. Please create a tutor profile first.',
      });
    }

    // Check if availability already exists
    const existingAvailability = await Availability.findOne({ tutorId: tutor._id });
 ...
 */
function generateSlotsForTimeRange(date, startTime, endTime, slotDurationMinutes) {
  const slots = [];
...
export const updateAvailability = async (req, res, next) => {
  try {
    ...
    if (!availability) {
      return res.status(404).json({
        message: 'Availability not found. Use POST to create availability first.',
      });
    }
