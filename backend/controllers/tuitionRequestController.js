...
export const createTuitionRequest = async (req, res, next) => {
  try {
    ...
    const { subject, budget, mode, description } = req.body;
...
export const getActiveTuitionRequestsForTutor = async (req, res, next) => {
  try {
    ...
    const requests = await TuitionRequest.find({ status: 'ACTIVE' })
      .sort({ createdAt: -1 })
      .select('subject budget mode description status createdAt')
      .lean();
