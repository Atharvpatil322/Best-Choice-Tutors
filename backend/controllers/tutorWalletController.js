 ...
 * Read-only wallet summary from TutorEarnings ledger. No withdrawals, no manual updates.
 *
 * Wallet lifecycle rule 4: Summary MUST compute:
 * - pendingEarnings = sum(entries where status === "pendingRelease")
 * - availableEarnings = sum(entries where status === "available")
...
export const getWallet = async (req, res, next) => {
  try {
    ...
    for (const e of entries) {
      ...
      else if (e.status === 'available') availableEarnings += e.amount;
    }
